...
@section('content')
    <div class="product-form__content">
        ...
        <form action="{{ route('register') }}" method="post" class="product-form" enctype="multipart/form-data">
            @csrf
            ...
            <div class="product-form__group">
                ...
                <input class="product-form__input" type="number" name="price" id="price" value="{{ old('price') }}"
                    placeholder="値段を入力">
                <div class="product-form__error-message">
                    @error('price')
                        {{ $message }}
                ...
                @foreach ($seasons as $season)
                    <label class="product-form__label" for="season_{{ $season->id }}">
                        <input class="product-form__input" type="checkbox" name="season_id[]" id="season_{{ $season->id }}"
                            value="{{ $season->id }}" {{ old('season_id') == $season->id ? 'checked' : '' }}>
